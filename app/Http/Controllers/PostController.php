...
class PostController extends Controller
{
    ...
    {
        // 1. Ambil input pencarian dan pilihan sort
        ...
        // 2. Simpan riwayat pencarian di session (maksimal 10 terakhir)
        if ($search !== '') {
        ...
        // 3. Query dasar dengan eager loading dan hitung relasi
        $like = '%' . $search . '%';
        $posts = Post::with(['user.badges', 'attachments', 'comments'])
            ->withCount(['upvotedBy', 'downvotedBy', 'bookmarkedBy', 'comments'])
        ...
        // 4. Hitung weighted_score dan set flags user
        $user = Auth::user();
        $posts->getCollection()->transform(function ($post) use ($user) {
