<x-layout>
    ...
    @if ($posts->isEmpty())
        <div class="w-full h-40 flex flex-col justify-center items-center text-center text-sl-text/80">
            <i class="fa-regular fa-face-frown text-4xl mb-2"></i>
            <p class="text-lg font-semibold">Belum ada komentar</p>
            <p class="text-sm text-gray-400">Kamu belum pernah mengomentari postingan apapun.</p>
        </div>
    @else
            ...
            <article class="min-w-full max-w-full w-full min-h-16 h-auto bg-sl-tertiary rounded-md flex flex-col p-3 gap-y-2">
                <section x-data="{ showOption: false }" class="w-full min-h-12 flex items-center justify-between relative">
                    ...
                    <div x-cloak x-show="showOption" @click.outside="showOption = false"
                        class="absolute top-0 right-0 w-20 h-auto bg-white/10 backdrop-blur-sm rounded-md shadow-lg flex flex-col gap-y-2 p-1 text-xs text-sl-text/90 z-50">
